...
testRule(S0, S1, S3, RE) :-
    testRule(S0, S1, S3, RE, 2).

testRule(S0, S1, S3, RE, USERULES) :-     %%%to get a  phonetic transcription with no stress value.
    retractall(useRules(_)),
    assert(useRules(USERULES)),
    string2chars(S0, S1),
    preprocess(S1, S2, USERULES),
    %% 4: use assimilation rules
    (USERULES /\ 4 > 0 ->
     applyImanRule(S2, S3);
     S3 = S2),
    (USERULES /\ 8 > 0 ->
    ...
    %% 16: apply stress
    (USERULES /\ 16 > 0 ->
     %% 64: do stress before assimilation
     (((USERULES /\ 64) > 0 -> S5 = S2; S5 = S4),
      (resplitWords(S5, RWORDS),
       assignStressToWords(RWORDS)));
     true),
    findall([sampa:char@X, final:pos@word:char@X, {vowel:char@X, stress:syll@syllable:char@X}], (member(X, S4), (sound(SOUND, X) -> true; fail)), SN),
    list_string(SN, RE).
...
listOfAtomsToString([[H0,  F, S] | T0], C) :-
   	atom_codes(H0, H1),
	(S == {+, +} ->
	 (useRules(USERULES),
	  %% 128 decorate the current phone with a stress marker
	  %% ow insert the stress marker as a separate syllable
	  (USERULES /\ 128 > 0 ->
	   F0 = "^^";
	   F0 = " ^^"));
	 F0 = []),
...
preprocess(TXT0, TXTN, USERULES) :-
    fixTanween(TXT0, TXT1),
    fixFem(TXT1, TXT2),
    remshadda(TXT2, TXT3),
    remsukuns(TXT3, TXT4),
    remdash(TXT4, TXT5),
    fixboundaries(TXT5, TXT6),
    fixHamza(TXT6, TXT7),
    fixsv(TXT7),
    !,
    %% Do pharangyealisation if USERULES has 32 set
    (0 is USERULES /\ 32 ->
     TXTN = TXT7;
     fixph(TXT7, TXTN)).
...
include_pa([N, B| T0], [M, B| T1]):-
	N <> invsound('n'),
	B <> invsound('b'),
	M <> invsound('m'),
	!,
	word:char@N <-> word:char@M,
	include_pa(T0, T1).

include_pa([N, F| T0], [M, F| T1]):-
	N <> invsound('n'),
	F <> invsound('f'),
	M <> invsound('M'),
	!,
...
include_pa([N, Q| T0], [C, Q| T1]):-
	N <> invsound('n'),
	(Q <> invsound('q'); Q <> invsound('k')),
	C <> invsound('c'),
	!,
	word:char@N <-> word:char@C,
	include_pa(T0, T1).

include_pa([N, T| T0], [E, T| T1]):-
	N <> invsound('n'),
	(T<>[dentfric]:char; T<>[alvstop]:char; T<>[alvfric]:char; T<>[postalv]:char),
	E <> invsound('e'),
	!,
	word:char@N <-> word:char@E,
	include_pa(T0, T1).

include_pa([H | T0], [H | T1]) :-
	include_pa(T0, T1).   
...
applyImanRule(LEFT, SURFACE, UNDERLYINGN) :-
	...
	((flag(showImanRules); SURFACE = UNDERLYING0) ->
...
applyImanRule(S, U) :-
 ...
 [{v0, artic=A},'A' , a, {alvlateral}, 'A', {shortV, -open, -rounded, word=WW}, {c0, grapheme=G, artic= B, word=WWW}] %%% IF  'ALA' WAS PROCEEDED BY A VOWEL %%%%% 
  ==>
  [{v1, shortV, artic=A, word=W}, {alvlateral, word=W1}, {shortV, -open, -rounded, word=WW}, {c1, grapheme=G, artic= B, word=WWW} ] :
  [???] ## [ ???]:-
 ...
 [{v0, shortV},{'A', word=WW} , a, {alvlateral}, {c1, grapheme=G, artic=AA,  +solar}]  %%% when preceeded by a prepositional           
 ==>
 ...
 [???] ## [???]:-
  ...
  [{v0, shortV}, {'A', word=WWW}, a, {alvlateral},  {c0, grapheme=G, -solar}]         %%% when preceeded by a prepositional             
  ==>
 ...
 [{c0, grapheme=G}, {'A',word=W},a, {alvlateral}, 'A']           %%%%% IF  'ALA' WAS PROCEEDED BY A CONSONANT %%%%%                 
 ==>
...
%% Allan, 21/08/2017: had to force the 'A' to belong V, otherwise I was getting
%% into a loop: should probably be fixed by fixing 'A' to be long in chartypes
%%%Iman, I had to delete ({shortV, -open, -rounded})that comes after c5.
[{c4, grapheme=GG, word=W}, {'A'},a,  {alvlateral} , {c0, grapheme= G, artic=AA, +solar}]    %%modified               
==>
[{c5, grapheme=GG, word=WW}, {c1, grapheme=G, sampa= X, artic=AA, +solar, word=W1}, {c2, grapheme= G, sampa= X, artic=AA, +solar, word=W2}]:
[???] ## [ ???]:-
	+final:pos@W,
	-final:pos@WW,
...
[???] ## [???]:-
 ...
 [{v0, shortV},{'A', word= WW}, {v1, closeV}, {c0, grapheme=G, artic=B}, {c1, grapheme=G1, artic=B1}] %%% IF HAMZAT ALWASL  WAS PROCEEDED BY A PREPOSITIONAL %%%%%   %%% OK%%%
  ...
  [{v0, artic=A},{'A', word=WW}, {v1, closeV}, {c0, grapheme=G, artic=B}, {c1, grapheme=G1, artic=B1}] %%% IF HAMZAT ALWASL  WAS PROCEEDED BY A VOWEL %%%%%   %%% OK%%% modified
  ==>
  [{v2, shortV, artic=A, word=W2}, {c2, grapheme=G, artic=B, word=W}, {c3, grapheme=G1, artic=B1, word=W1}] :
  [???] ## [ v3, ???]:-
	  +initial:pos@WW,
	  +final:pos@W, -initial:pos@W,
  	  -final:pos@W1, +initial:pos@W1,
  ...
  ==>
  [{c5, grapheme=G4, word=W4},{v0, closeV}, {c2, grapheme=G, artic=A, word=W1}, {c3, grapheme=G1, artic=A1, word=W2}] :
  [???] ## [ v1, ???] :-
  	+initial:pos@W,
...
%%% totl assimilation
  ...
  [{c0, grapheme=G0, alvstop, -emph, word=W}]      %% for /t/ and /d/                            
  ==>
  [{c1, alvstop, grapheme= G1, word=W}]:
  [???] ## [{c2, alvstop, grapheme=G1}, ???]:-
      	\+ G0 =  G1.
